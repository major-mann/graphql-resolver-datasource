module.exports = createMongooseDatasource;
...
function createMongooseDatasource(collection, createKey, autoGenerate) {
    ...
    function statsWrap(name, resolver) {
        const beginStatName = `datasource.mongoose.${name}.begin`;
        const completeStatName = `datasource.mongoose.${name}.complete`;
        const failStatName = `datasource.mongoose.${name}.fail`;
        const timingStatName = `datasource.mongoose.${name}.time`;
